...
mac 安装时：打开 `.dmg`，把 `SVT Pets.app` 拖到 `Applications`。

如果提示“无法打开/无法验证开发者”，用右键 'SVT Pets.app` -> 打开，或到系统设置里允许。
